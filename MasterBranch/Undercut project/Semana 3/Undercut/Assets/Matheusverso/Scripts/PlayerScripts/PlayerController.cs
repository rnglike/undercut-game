...
public class PlayerController : MonoBehaviour
{
    ...
    public float startFreeThePlayerTime;
    ...
    void Update()
    {
        if(!inside && building.currentRoom > 1)
        {
            Debug.Log("Dead");
            Warps(0, 0, 0);
        }
    ...
    // Warps player to (x,y,z)
    public void Warps(float x, float y, float z)
    {
    ...
    {
        if((Input.GetButtonDown("Interact")) && (any.transform.parent.tag == "Door"))
        {
            building.DoorWarp(transform,any.gameObject.GetComponent<Door>().downStairs);
        }
    }


    // Comando que libera o jogador do Lock do inimigo, "durationtimelocked" do script Enemy precisa ser IGUAL ao "startFreeThePlayerTime" do player pra funcionar legal. Alterando no menu do Unity mesmo.
    IEnumerator FreeThePlayerDelay()
    {
        yield return new WaitForSeconds(startFreeThePlayerTime);
        FreeThePlayer();
    }
